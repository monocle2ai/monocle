...
- feat: capture response_format in data.input of litellm instrumentation ([#637](https://github.com/monocle2ai/monocle/pull/637))
- feat: add token-summary CLI command for local trace files ([#618](https://github.com/monocle2ai/monocle/pull/618))
- docs: add Discord invite badge to README ([#623](https://github.com/monocle2ai/monocle/pull/623))
- feat: assert specific or total invocation counts for agents and tools ([#619](https://github.com/monocle2ai/monocle/pull/619))
- feat: updated test tools and readme for the TestGenerator functionality ([#614](https://github.com/monocle2ai/monocle/pull/614))
- fix: terminal picker resize + Copilot OTel docs ([#615](https://github.com/monocle2ai/monocle/pull/615))
- feat: add support to Agentic CLI instrumentation to support tracking code change metrics ([#597](https://github.com/monocle2ai/monocle/pull/597))
- docs: animated logo, add JSON/scope/multi-turn examples, fix Discord link ([#617](https://github.com/monocle2ai/monocle/pull/617))
- fix: handle propagated parent traces and ensure workflow span closure on child errors ([#613](https://github.com/monocle2ai/monocle/pull/613))
- feat: add core non-LLM evals (regex, JSON validity, keyword presence, exact match) ([#612](https://github.com/monocle2ai/monocle/pull/612))
- fix: skip litellm inference span if the async flag is true for sync method ([#609](https://github.com/monocle2ai/monocle/pull/609))
- feat: support generating test from trace, scan the trace and generate trace with assertions ([#610](https://github.com/monocle2ai/monocle/pull/610))
- fix: removed arbitrary time filtering for okahu eval ([#602](https://github.com/monocle2ai/monocle/pull/602))
- feat: json and okahu loader pytest examples with all types of pytest assertions ([#608](https://github.com/monocle2ai/monocle/pull/608))
- feat(exporters): configurable file-name prefix for file and Azure Blob exporters via `MONOCLE_FILE_PREFIX` and `MONOCLE_BLOB_FILE_PREFIX`; S3 `MONOCLE_S3_KEY_PREFIX` renamed to `MONOCLE_S3_FILE_PREFIX` ([#149](https://github.com/monocle2ai/monocle/issues/149))
