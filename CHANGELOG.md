...
- feat(test_tools): custom validators - `check_validator` runs the test author's own pass/fail logic over the recorded responses, for the checks no grader can make. Each selected span's recorded `input`/`output` is passed to the validator, which returns `True`, or `False`/a message that becomes the assertion message. Takes a function, a `BaseValidator` subclass, or an import path to either; needs no evaluator and makes no network call. `FluentTestCase` gained a `validators` field, so `check_validator(testcase=...)` works like `check_eval(testcase=...)`
