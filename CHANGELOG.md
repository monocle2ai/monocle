...
- feat(test_tools): `called_any_agent(*agent_names)` and `called_any_tool(*tool_names, agent_name=None)` assert that **any** of the named agents/tools was invoked, for runs where more than one route is acceptable. Names may be passed separately or as a single list; the context narrows to the union of the matched entities' spans in trace order, so the chained checks see the whole set, and `count`/`min_count`/`max_count` constrain the total invocations across them. Their negatives, `does_not_call_any_agent` / `does_not_call_any_tool`, assert that **none** of the named entities ran — one call to any of them fails the assertion, and the failure names what ran rather than the whole candidate list
- feat(test_tools): add the `a2a` runner, which tests an agent over the A2A protocol by driving it as an A2A client (installed with the `a2a` extra). The agent's spans join the test's trace: set `A2A_TRACE_WORKFLOW` and the runner pulls them from Okahu, leave it unset and the test reads the agent's trace file. `a2a_transport.make_traced_httpx_client` gives the same propagation to an agent that calls another agent over A2A
- feat(instrumentation): instrument the httpx client, as `requests` already is — a call carries the trace context out, so the service it reaches continues the caller's trace, and spans a trace-returning service appends to its response are taken off it. Only hosts named in `MONOCLE_TRACE_PROPAGATATION_URLS` are traced, so an app's other httpx traffic is untouched
- feat(instrumentation): `collect_returned_spans()` hands a caller the spans a service returned, for the common case where an SDK owns the response and the caller never sees it. Works for `requests` and httpx alike
- feat(test_tools): the `a2a` runner gets the agent's spans back with its answer when the agent has trace return on (`MONOCLE_ENABLE_TRACE_RETURN` plus a key matching the test's `MONOCLE_TRACE_RETRIEVAL_KEY`), so a remote agent can be asserted on with no trace backend and no access to its filesystem. Nothing is then pulled from Okahu, which would count each span twice
- fix(test_tools): a span the test already holds is not added again — an agent can both return its spans with its answer and export them, and reading both routes counted every span twice, doubling every count assertion
- fix(instrumentation): the A2A client span records the call again — `get_url` read a field the current a2a SDK does not have, and the `status` and `response` accessors raised on a Message result, an error, or a task with no artifacts. The span handler drops accessor errors silently, so the attributes were simply missing
...
- fix(exporters): span obfuscation falls back to the built-in `credentials` obfuscator when none of the configured ones can be loaded, instead of exporting payloads unscrubbed. Previously `MONOCLE_SPAN_OBFUSCATORS=presidio` without the `obfuscation` extra installed silently disabled obfuscation altogether; listing it alongside another obfuscator was unaffected. Only an explicit opt-out (`MONOCLE_DISABLE_SPAN_OBFUSCATION`, an off-switch value, or `span_obfuscators=[]`) turns scrubbing off
