...
- fix(exporters): span obfuscation recognises Okahu API keys (`okh_…`). Monocle exports to Okahu, so its key is the one most likely to reach a span, and it was the only major vendor shape missing. It was redacted when passed by name (`api_key=…`, which `credential_assignment` matches) but exported in the clear when passed positionally, where no name is available and only the shape can give it away
- feat(exporters): span obfuscation replaces inline media with its type and size, so a multimodal call exports `"url": "<IMAGE:image/png,1.4MB>"` instead of megabytes of base64. Previously the whole encoded image, audio clip or PDF landed in `data.input` verbatim, matched by no credential pattern. Covers OpenAI's `data:` URLs (`data_url`) and the bare base64 Anthropic and Gemini send (`base64_blob`, any unbroken run of 512+ characters). Turn it off with `patterns=list(CREDENTIAL_PATTERNS)`
- fix(exporters): span obfuscation redacts a credential named by its payload key, not just one recognizable from its text — `{"api_key": "hunter2"}` now exports `"api_key": "<REDACTED>"`, where previously a bare secret with no `key=value` syntax was exported in the clear. Only a custom output processor produces such a key. The new `credential_key` pattern runs last, so `{"api_key": "sk-proj-…"}` keeps its precise `<API_KEY>` marker
