...
class CopilotSpanHandler(SpanHandler):
    @staticmethod
    ...
    def _prepare_span_context(self, kwargs):
        ...
        if start_time:
            token_context = set_value(SPAN_START_TIME, CopilotSpanHandler._iso_to_ns(start_time), token_context)
        if end_time:
            token_context = set_value(SPAN_END_TIME, CopilotSpanHandler._iso_to_ns(end_time), token_context)
        scopes = {}
        if COPILOT_TURN_SCOPE in kwargs:
            scopes[COPILOT_TURN_SCOPE] = kwargs.pop(COPILOT_TURN_SCOPE)
