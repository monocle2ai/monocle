...
def agent_inference_type(arguments):
    ...
    try:
        ...
        if status in ('success', 'completed'):
            ...
            # Handle stream-processor output namespace (chat.stream / stream_async)
            if hasattr(response, "output_text") and hasattr(response, "finish_reason"):
                if response.finish_reason == "tool_calls" and getattr(response, "tools", None):
...
def extract_tool_name(arguments):
    ...
    try:
        ...
        for getter in [
            lambda tc: tc.get("name") if isinstance(tc, dict) else None,  # streaming: tool dict
            lambda tc: tc.function.name,                                   # non-streaming: ToolCall object
        ]:
            try:
                name = getter(tool_call)
                if name:
                    return name
