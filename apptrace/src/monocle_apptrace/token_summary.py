...
_SESSION_HEADERS = ["Session", "Model", "Input", "Cache Read", "Cache Create", "Output", "Total"]


def summarize_by_session(time_window="all", monocle_dir=None):
    # type: (str, Optional[Path]) -> List[Dict]
...
def format_session_table(rows):
    # type: (List[Dict]) -> str
    """Render per-session *rows* as a plain-text ASCII table."""
    if not rows:
        return "No trace files found for the given time window."

    col_keys = [
        "session", "model", "prompt_tokens",
        "cache_read_input_tokens", "cache_creation_input_tokens",
        "completion_tokens", "total_tokens",
    ]
    widths = [len(h) for h in _SESSION_HEADERS]
    for row in rows:
        for i, key in enumerate(col_keys):
            widths[i] = max(widths[i], len(str(row[key])))
    ...
    lines = [sep, fmt.format(*_SESSION_HEADERS), sep]
    for row in rows:
        lines.append(fmt.format(*[str(row[k]) for k in col_keys]))
    lines.append(sep)
    return "\n".join(lines)