...
def setup():
    ...
    try:
        instrumentor = setup_monocle_telemetry(
            workflow_name="litellm_bedrock_app_1",
...
# AWS Bedrock model id (Converse route). The provider prefix is stripped before
# it reaches the instrumented BedrockConverseLLM, so the span records the bare id.
...
@pytest.mark.unit()
def test_llm_bedrock(setup):
    if os.getenv("AWS_ACCESS_KEY_ID") is None and os.getenv("AWS_PROFILE") is None:
        raise ValueError("AWS credentials (AWS_ACCESS_KEY_ID or AWS_PROFILE) must be set.")
