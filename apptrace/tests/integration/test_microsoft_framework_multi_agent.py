...
# @pytest.mark.asyncio
# async def test_microsoft_multi_agent(setup):
#     """Multi-agent Microsoft Agent Framework test with 3 agents and 2 tools."""
...
#     task_description = "Book a flight from San Francisco to New York next week, and book a Marriott hotel in New York for 2 nights."
    
#     logger.info(f"Task: {task_description}")
    
#     # First, execute flight booking agent
#     flight_response = ""
#     async for chunk in flight_agent.run_stream(task_description):
#         if chunk.text:
#             flight_response += chunk.text
#     logger.info(f"Flight Agent Response: {flight_response}")
    
#     # Then, execute hotel booking agent
#     hotel_response = ""
#     async for chunk in hotel_agent.run_stream(task_description):
#         if chunk.text:
#             hotel_response += chunk.text
#     logger.info(f"Hotel Agent Response: {hotel_response}")
    
#     # Finally, execute supervisor to coordinate
#     supervisor_task = f"Coordinate and summarize these bookings:\nFlight: {flight_response}\nHotel: {hotel_response}"
#     supervisor_response = ""
#     async for chunk in supervisor_agent.run_stream(supervisor_task):
#         if chunk.text:
#             supervisor_response += chunk.text
#     logger.info(f"Supervisor Response: {supervisor_response}")
    
#     # Basic verification
#     assert flight_response, "Should get flight response"
#     assert hotel_response, "Should get hotel response"
#     assert supervisor_response, "Should get supervisor response"
    
#     # Verify bookings are mentioned
#     assert "FLIGHT" in flight_response.upper() or "flight" in flight_response.lower(), "Should contain flight booking"
#     assert "HOTEL" in hotel_response.upper() or "hotel" in hotel_response.lower(), "Should contain hotel booking"
...
#             elif span_attributes["entity.1.name"] == "MS_Hotel_Booking_Agent":
#                 found_hotel_agent = True
#             elif span_attributes["entity.1.name"] == "MS_Travel_Supervisor":
