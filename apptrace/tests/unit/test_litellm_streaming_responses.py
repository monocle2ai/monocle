...
def test_extract_assistant_message_stream_result():
    # LiteLLMStreamProcessor stream results: tools as {"name", "args"}
    result = SimpleNamespace(
        tools=[{"name": "finish", "args": '{"message":"done"}'}],
        output_text="",
    )
...
def test_is_streaming_request_reads_kwargs_and_optional_params():
    assert _helper.is_streaming_request({"stream": True}) is True
    assert _helper.is_streaming_request({"optional_params": {"stream": True}}) is True
    assert _helper.is_streaming_request({"optional_params": {}}) is False
    assert _helper.is_streaming_request({}) is False
...
def test_handle_event_assembles_streamed_responses_tool_calls():
    ...
    # stream namespace with tools: finish reason and output extraction
    ns = SimpleNamespace(output_text="", tools=state.tools)
    assert _helper.extract_responses_finish_reason({"result": ns, "exception": None}) == "tool_calls"
    out = _helper.extract_responses_output({"result": ns, "exception": None})
