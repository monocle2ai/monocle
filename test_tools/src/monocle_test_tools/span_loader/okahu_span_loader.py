...
class OkahuSpanLoader:
    ...
    @staticmethod
    def record_test_result(test_name:str, test_failed:bool, test_assertion_message:str, trace_id:str,
                                        workflow_name:str, fact_name="traces"):
        """Record a test's pass/fail outcome against its remote (Okahu) trace using Okahu's evaluation API."""

        api_key = (os.getenv("OKAHU_API_KEY") or "").strip()
        if not api_key:
            logger.warning("OKAHU_API_KEY is not configured; skipping test result recording for '%s'.", test_name)
            return

        headers = {"x-api-key": api_key}

        test_label = f"{test_name}: PASSED" if not test_failed else f"{test_name}: FAILED"
        payload = {
            "template_name": test_name,
            "pre_calculated_result": {
                "label": test_label,
                "explanation": test_assertion_message or ("Test failed" if test_failed else "Test passed"),
            },
        }
        params = {
            "workflow_name": workflow_name,
        ...
        try:
            OkahuEval.submit_eval_job(headers=headers, payload=payload, params=params)
        except Exception as exc:
            # Recording is a side effect of cleanup; never let it alter the test outcome.
            logger.warning("Failed to record test result for '%s' (trace %s): %s", test_name, trace_id, exc)