...
from monocle_apptrace.instrumentation.common.constants import (
    MONOCLE_SKIP_EXECUTIONS,
    MONOCLE_WORKFLOW_NAME_KEY,
)
from monocle_apptrace.instrumentation.common.instrumentor import (
    MonocleInstrumentor,
    get_monocle_instrumentor,
    reset_span_processors,
...
from monocle_apptrace.instrumentation.metamodel.langgraph.entities.inference import (
    TOOLS as LANGGRAPH_TOOL,
)
from monocle_apptrace.instrumentation.metamodel.langgraph.methods import (
    LANGGRAPH_METHODS,
)
...
from monocle_test_tools.schema import (
    EvalInputs,
    Evaluation,
    MockTool,
    SpanType,
...
class MonocleValidator:
    ...
    def __init__(
        self,
        exporter_list: Optional[str] = None,
        export_failed_tests_only: Optional[bool] = None,
        workflow_name: str = None,
    ):
        if MonocleValidator._initialized:
            if exporter_list is not None:
                raise ValueError(
                    "Exporter list can only be set during the first initialization of MonocleValidator."
                )
            return
        ...
        if export_failed_tests_only is None:
            export_failed_tests_only = (
                os.getenv("MONOCLE_EXPORT_FAILED_TESTS_ONLY", "false").lower() == "true"
            )
    ...
    def flush_to_exporters(
        self, test_name: str, test_failed: bool, test_assertion_message: str = None
    ):
            ...
            for span in self.memory_exporter.get_finished_spans():
                if test_failed:
                    ...
                    if test_assertion_message is not None:
                        span._attributes[TEST_ASSERTION_ATTRIBUTE] = (
    ...
    def pre_test_run_setup(
        self, test_case_name: str, mock_tools: Optional[list[MockTool]] = None
    ) -> None:
        """
    ...
    def post_test_cleanup(
        ...
        test_name: str,
        test_failed: bool,
        test_assertion_message: str = None,
    ) -> None:
        try:
    ...
    @contextmanager
    def monocle_exporter_wrapper(
        self, test_case: TestCase, request: pytest.FixtureRequest
    ):
        test_case_name = (
            request.node.name
            if request is not None
            else (test_case.test_case_name if test_case is not None else "monocle_test")
        )
        ...
        try:
            ...
            try:
                ...
                test_failed = validation_failed or (
                    request.session.testsfailed > prior_test_failed_count
                )
                self.post_test_cleanup(
                    token, request.node.name, test_failed, validation_error_message
                )
                self._spans = []
                if token is not None:
                    stop_scope(token)
    ...
    async def test_workflow_async(
        self, workflow_func, test_case: Union[TestCase, dict]
    ):
    ...
    async def run_agent_async(
        agent, agent_type: str, *args, session_id: str = None, **kwargs
    ):
        agent_runner = get_agent_runner(agent_type)
        if agent_runner is None:
            raise ValueError(f"Unsupported agent type: {agent_type}")
        result = await agent_runner.run_agent_async(
            agent, *args, session_id=session_id, **kwargs
        )
        return result
    ...
    async def test_agent_async(
        ...
        result = None
        try:
            result = await MonocleValidator.run_agent_async(
                agent, agent_type, *test_case.test_input, session_id=session_id
            )
        except Exception as e:
            if not test_case.expect_errors:
                raise
    ...
    def test_agent(self, agent, agent_type: str, test_case: Union[TestCase, dict]):
        ...
        result = None
        try:
            result = MonocleValidator.run_agent(
                agent, agent_type, *test_case.test_input
            )
        except Exception as e:
            if not test_case.expect_errors:
                raise
    ...
    def validate(self, test_case: TestCase) -> bool:
        ...
        for test_span in test_case.test_spans:
            if test_span.span_type == SpanType.TOOL_INVOCATION:
                ...
                self.verify_agent_delegated(
                    from_agent,
                    to_agent,
                    test_span.positive_test,
                    test_span.expect_errors,
                    test_span.expect_warnings,
                )
    ...
    def validate_result(self, test_case: TestCase, result) -> bool:
        if test_case.test_output is not None:
            assert test_case.comparer.compare(
    ...
    def verify_agentic_request(self, test_span: TestSpan) -> bool:
        ...
        agent_request_span = self._get_agent_request_span(
            expect_errors, expect_warnings
        )

        actual_output = self._agent_request_output(
            agent_request_span, expect_errors, expect_warnings
        )
        assert actual_output is not None, "No response found in agent request span."
        if expected_response is not None:
            valid_response: bool = self._valid_response(
                expected_response, actual_output.get("output"), comparer
            )
        ...
        if expected_request is not None:
            valid_input: bool = self._valid_response(
                expected_request, actual_output.get("input"), comparer
            )
    ...
    def verify_inference(self, test_span: TestSpan) -> bool:
        ...
        for inference_span in inference_spans:
            ...
            if expected_schema is not None and not verified_schema:
                if isinstance(expected_schema, dict):
                    try:
                        jsonschema.validate(
                            instance=json.loads(inference_response),
                ...
                if not expect_errors and self._span_has_error(inference_span):
                    assert (
                        False
                    ), f"Inference matched the expected schema but had errors."
                if not expect_warnings and self._span_has_warning(inference_span):
                    assert (
                        False
                    ), f"Inference matched the expected schema but had warnings."
        ...
        if expected_schema is not None and not verified_schema and positive_test:
            ...
            assert (
                False
            ), f"An inference matched the expected schema, but was not expected to be."
        ...
        if expected_output is not None and not verified_response and positive_test:
            ...
            assert (
                False
            ), f"An inference matched the expected response, but was not expected to be."

        if (
            eval is not None
            and verified_response
            and verified_response_span is not None
        ):
    ...
    def _valid_response(
        self, response_to_evaluate: str, actual_response: str, comparer: BaseComparer
    ) -> bool:
    ...
    def verify_tool_invoked(self, test_span: TestSpan) -> bool:
        ...
        agent_name: str = (
            test_span.entities[1].name if len(test_span.entities) > 1 else None
        )
        tool_inputs: list[str] = (
            [test_span.input] if test_span.input is not None else []
        )
        tool_outputs: list[str] = (
            [test_span.output] if test_span.output is not None else []
        )
        positive_test: bool = test_span.positive_test
        ...
        if len(tool_invocation_spans) == 0:
            if positive_test:
                assert (
                    False
                ), f"Tool '{tool_name}' was not invoked by agent '{agent_name}'."
            return True

        if not positive_test:
            assert (
                False
            ), f"Tool '{tool_name}' was invoked by agent '{agent_name}', but was not expected to be."

        self._check_error_warnings(
            tool_invocation_spans, tool_name, agent_name, expect_error, expect_warnings
        )
    ...
    def verify_agent_invoked(self, test_span: TestSpan) -> bool:
        ...
        if not positive_test:
            assert (
                False
            ), f"Agent '{agent_name}' was invoked, but was not expected to be."
        self._check_error_warnings(
            agent_invocation_spans, None, agent_name, expect_error, expect_warnings
        )
        self._check_input_output(
        ...
        for span in self.spans:
            ...
            if (
                ...
                if not positive_test:
                    assert (
                        False
                    ), f"Agent '{to_agent}' was delegated by '{from_agent}', but was not expected to be."
                if not expect_error and self._span_has_error(span):
                    assert (
                        False
                    ), f"Agent '{to_agent}' was delegated by '{from_agent}' but error found."
                if expect_warnings and not self._span_has_warning(span):
                    assert (
                        False
                    ), f"Agent '{to_agent}' was delegated by '{from_agent}' warning was expected but no warning found."
    ...
    def check_completion_token_limits(
        self,
        max_output_tokens: int,
        positive_test: bool = True,
        filtered_spans: Optional[list[Span]] = None,
    ) -> bool:
        ...
        if max_output_tokens is None:
            return True
        return self._check_token_limits(
            max_output_tokens, "completion_tokens", positive_test, filtered_spans
        )

    def check_total_token_limits(
        self,
    ...
    def _check_token_limits(
        ...
        if positive_test:
            assert tokens <= max_tokens, (
                custom_message
                if custom_message
                else f" {token_name} limit exceeded: {tokens} > {max_tokens}"
            )
        else:
            assert tokens > max_tokens, (
                custom_message
                if custom_message
                else f" {token_name} limit was not exceeded as expected: {tokens} <= {max_tokens}"
            )
        return True

    def check_total_duration_limits(
        self,
        ...
        if not spans_to_check or len(spans_to_check) == 0:
            assert False, (
                custom_message
                if custom_message
                else "No spans available to check workflow duration."
            )
        ...
        if workflow_span is None:
            assert False, (
                custom_message
                if custom_message
                else "No workflow span found (span with name 'workflow')."
            )
        ...
        if positive_test:
            assert duration_seconds <= max_duration_seconds, (
                custom_message
                if custom_message
                else f"Workflow duration {duration_seconds:.2f}s exceeds limit {max_duration_seconds}s."
            )
        else:
            assert duration_seconds > max_duration_seconds, (
                custom_message
                if custom_message
                else f"Workflow duration {duration_seconds:.2f}s was expected to exceed limit {max_duration_seconds}s but did not."
            )

        return True

    def _verify_tool_errors(
        self,
        tool_name: str,
        agent_name: str,
        expect_error: bool,
        ...
        if expect_error and not found_error:
            assert (
                False
            ), f"Tool '{tool_name}' was invoked by agent '{agent_name}' error was expected but no error found."
        elif not expect_error and found_error:
            assert (
                False
            ), f"Tool '{tool_name}' was invoked by agent '{agent_name}' but error found."
        if expect_warnings and not found_warning:
            assert (
                False
            ), f"Tool '{tool_name}' was invoked by agent '{agent_name}' warning was expected but no warning found."
        elif not expect_warnings and found_warning:
            assert (
                False
            ), f"Tool '{tool_name}' was invoked by agent '{agent_name}' but warning found."

    def _verify_tool_input_output(
        self,
        tool_name: str,
        agent_name: str,
        tool_inputs: Optional[list[str]],
        found_input: bool,
        ...
        if len(tool_inputs) > 0 and found_input is False and positive_test:
            assert (
                False
            ), f"Tool '{tool_name}' was never invoked by agent {agent_name} with input '{tool_inputs}'."
        elif len(tool_inputs) > 0 and found_input is True and not positive_test:
            assert (
                False
            ), f"Tool '{tool_name}' was invoked by agent {agent_name} with input '{tool_inputs}', but was not expected to be."

        if len(tool_outputs) > 0 and found_output is False and positive_test:
            assert (
                False
            ), f"Tool '{tool_name}' invoked by agent {agent_name} didn't return '{tool_outputs}'."
        elif len(tool_outputs) > 0 and found_output is True and not positive_test:
            assert (
                False
            ), f"Tool '{tool_name}' invoked by agent {agent_name} returned '{tool_outputs}', but was not expected to be."

    def _verify_agent_input_output(
        self,
        agent_name: str,
        agent_inputs: Optional[list[str]],
        found_input: bool,
        agent_outputs: Optional[list[str]],
        found_output: bool,
        positive_test: bool,
    ) -> None:
        if len(agent_inputs) > 0 and found_input is False and positive_test:
            assert (
                False
            ), f"Agent '{agent_name}' was never invoked with input '{agent_inputs}'."
        elif len(agent_inputs) > 0 and found_input is True and not positive_test:
            assert (
                False
            ), f"Agent '{agent_name}' was invoked with input '{agent_inputs}', but was not expected to be."
        ...
        if len(agent_outputs) > 0 and found_output is False and positive_test:
            ...
            assert (
                False
            ), f"Agent '{agent_name}' returned '{agent_outputs}', but was not expected to be."

    def _check_error_warnings(
        self,
        spans: list[Span],
        ...
        if tool_name is not None:
            ...
            self._verify_agent_errors(
                agent_name, expect_error, found_error, expect_warnings, found_warning
            )

    def _verify_agent_errors(
        self,
        agent_name: str,
        expect_error: bool,
        found_error: bool,
        ...
        if expect_error and not found_error:
            assert (
                False
            ), f"Agent '{agent_name}' was invoked error was expected but no error found."
        elif not expect_error and found_error:
            assert False, f"Agent '{agent_name}' was invoked but error found."
        if expect_warnings and not found_warning:
            assert (
                False
            ), f"Agent '{agent_name}' was invoked warning was expected but no warning found."
        elif not expect_warnings and found_warning:
    ...
    def _check_input_output(
        ...
        if len(expected_inputs) > 0 or len(expected_outputs) > 0:
                ...
                for event in span.events:
                    if event.name == "data.input":
                        for expected_input in expected_inputs:
                            if comparer.compare(
                                expected_input, event.attributes.get("input")
                            ):
                        ...
                        for expected_output in expected_outputs:
                            if comparer.compare(
                                expected_output, event.attributes.get("response")
                            ):
            ...
            if tool_name is not None:
                self._verify_tool_input_output(
                    tool_name,
                    agent_name,
                    expected_inputs,
                    found_input,
                ...
                self._verify_agent_input_output(
                    agent_name,
                    expected_inputs,
                    found_input,
                    expected_outputs,
    ...
    def _get_inference_spans(
        self, expect_errors: bool = False, expect_warnings: bool = False
    ) -> list[Span]:
        ...
        for span in self.spans:
            ...
            if "span.type" in span_attributes and (
                ...
                # Check for errors and warnings if expected
                found_error = self._span_has_error(span)
                if expect_errors and not found_error:
                    continue  # Skip spans without errors if errors are expected
                elif not expect_errors and found_error:
                    continue  # Skip spans with errors if errors are not expected

                found_warning = self._span_has_warning(span)
                if expect_warnings and not found_warning:
                    continue  # Skip spans without warnings if warnings are expected
                elif not expect_warnings and found_warning:
                    continue  # Skip spans with warnings if warnings are not expected
    ...
    def _get_agent_request_span(
        self, expect_error: bool = False, expect_warnings: bool = False
    ) -> Optional[Span]:
        for span in self.spans:
            span_attributes = span.attributes
            if (
                ...
                if expect_error and not found_error:
                    assert (
                        False
                    ), f"Agent request span error was expected but no error found."
                elif not expect_error and found_error:
                ...
                if expect_warnings and not found_warning:
                    assert (
                        False
                    ), f"Agent request span warning was expected but no warning found."
                elif not expect_warnings and found_warning:
                    assert False, f"Agent request span warning found."
                return span
        return None

    def _get_tool_invocation_spans(
        self,
        tool_name: str,
        agent_name: str = None,
        filtered_spans: Optional[list[Span]] = None,
        ...
        for span in spans_to_check:
            ...
            if (
                ...
                if span_attributes.get("entity.1.name", "") == tool_name and (
                    agent_name is None
                    or (
                        agent_name is not None
                        and span_attributes.get("entity.2.name", "") == agent_name
                    )
                ):
                    tool_invocation_spans.append(span)
        return tool_invocation_spans

    def _get_agent_invocation_spans(
        self, agent_name: str, filtered_spans: Optional[list[Span]] = None
    ) -> list:
        agent_invocation_spans = []
        spans_to_check = filtered_spans if filtered_spans is not None else self.spans
        for span in spans_to_check:
    ...
    def _agent_request_output(
        self,
        agent_request_span: Span,
    ...
    def _evaluate_span(
        self, span: Span, evaluation: Evaluation, positive_test: bool
    ) -> None:
        eval_args = {}
        for arg in evaluation.args:
            if arg == EvalInputs.INPUT:
                ...
                eval_args["agent_description"] = (
                    trace_utils.get_agent_description_from_span(span)
                )
        actual_eval: dict = evaluation.eval.evaluate(eval_args)
        if positive_test and not evaluation.comparer.compare(
            evaluation.expected_result, actual_eval
        ):
            assert (
                False
            ), f"Span {span.name} evaluation failed. Expected: {evaluation.expected_result}, Actual: {actual_eval}"
        elif not positive_test and evaluation.comparer.compare(
            evaluation.expected_result, actual_eval
        ):
            assert (
                False
            ), f"Span {span.name} evaluation passed, but was not expected to. Actual: {actual_eval}"
